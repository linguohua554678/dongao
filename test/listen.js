...
define(function(require, exports, module) {
    ...
    function bindFirst(){
        $.getJSON('http://course1.dongao.com/groupkey', function (json) {
            var lst = eval(json);
            $("#first").html("");
    ...
    function bindSecond(id){
        $.getJSON('http://course1.dongao.com/groupitem?id='+id, function (json) {
            var lst = eval(json);
            $("#second").html("");
            for (i = 0; i < lst.length; i++) {
                var tname = lst[i].name;
    ...
    function init(){
       var  initType = "big";
        ...
        listenView.init({
            pageType:initType,
            url:"http://course1.dongao.com/lecture_content?id="+$("#requestURL").val(),
            flash:'DongAoPlayer'
        });
    }
    //听课页面对象
